...
from .forms import  AdminUpadteOrderForm, FontSectionForm, FeatureSectionForm, ContactSectionForm, GellerySectionForm, PhotoSectionForm, ProductDetailsFrom,FontSectionImageForm
...
def update_feature_section(request):
    ...
    if request.method == 'POST':
        form = FeatureSectionForm(request.POST,  )
        if form.is_valid():
            form.save()
...
def update_contact_section(request):
    # ContactSectionDetails = ContactSection.objects.get(id = 1)
    form = ContactSectionForm() #instance=ContactSectionDetails
    if request.method == 'POST':
        form = ContactSectionForm(request.POST,)
        if form.is_valid():
            form.save()
            return redirect('landing-page')
...
def update_gallery_section_data(request):
    ...
    form = GellerySectionForm() # instance=GellerySectionDetails
    
    if request.method == 'POST':
        form = GellerySectionForm(request.POST,)
...
def updateProductDetails(request):
    # product_details = ProductDetails.objects.get(id = 1)
    form = ProductDetailsFrom() #product_details
    
    if request.method == 'POST':
        form = ProductDetailsFrom(request.POST, )
...
def Admin_update_font_section(request):
    FontSectionDetails = FontSection.objects.get(id = 1)
    form = FontSectionImageForm(instance=FontSectionDetails) #(instance=FontSectionDetails)
    if request.method == 'POST':
        form = FontSectionImageForm(request.POST, request.FILES, instance=FontSectionDetails)
        if form.is_valid():
            # print(FontSectionDetails.font_asset)
            form.save()
            return redirect('content-dashboard')    
